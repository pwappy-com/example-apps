...
async function なろう詳細ページのShow(event){setLoadingIndicator(!0);const ncode=event.target.data.ncode;const title=event.target.data.title;const status=event.target.data.status;const episodeCount=event.target.data.episodeCount;const url=event.target.data.url;const readingHistory=JSON.parse(localStorage.getItem('readingHistory'))||{};const history=readingHistory[url];let currentPage=1;let currentRow=1;let totalRowCount=0;let totalEpisodeCount=0;if(history){currentPage=history.currentEpisode;currentRow=history.currentRow;totalRowCount=history.totalRowCount;totalEpisodeCount=episodeCount;history.totalEpisodeCount=episodeCount;localStorage.setItem('readingHistory',JSON.stringify(readingHistory))}
...
async function ページ選択(event){const page=event.target.closest('ons-page');const ncode=page.data.ncode;const status=page.data.status;const title=page.data.title;const episodeCount=page.data.episodeCount;const selectedPage=event.target.value;if(!selectedPage)return;document.getElementById('ons-list-header2').scrollIntoView();setLoadingIndicator(!0);try{let url=`${urlBase}/${ncode}/`;const aheadPages=JSON.parse(localStorage.getItem('aheadPages'))||{};if(aheadPages[url]&&aheadPages[url][selectedPage]){const compressData=aheadPages[url][selectedPage];const data=LZString.decompress(compressData);displayData(JSON.parse(data))}else{await fetchDataAndDisplayByPage(ncode,status,selectedPage)}
...
function 読み上げ開始(event){const detailList=document.getElementById('detail-list');const listItems=detailList.querySelectorAll('.list-item_p');const page=detailList.closest('ons-page');const ncode=detailList.closest('ons-page').data.ncode;ページ先読み(ncode);let currentIndex=0;const voiceSpeechingItems=detailList.querySelectorAll('.voice-speeching');if(voiceSpeechingItems.length>0){currentIndex=Array.from(listItems).indexOf(voiceSpeechingItems[0])}
function readNextItem(){if(currentIndex<listItems.length){const listItem=listItems[currentIndex];const text=listItem.textContent;const textToRead=text.replace(/\|([^《]+)《([^》]+)》/g,' $2');const utterance=new SpeechSynthesisUtterance(textToRead);utterance.lang="ja-JP";utterance.rate=parseFloat(localStorage.getItem('rate'))||1;utterance.volume=parseFloat(localStorage.getItem('volume'))/100;utterance.voice=speechSynthesis.getVoices().filter(voice=>voice.name===localStorage.getItem('voice'))[0];speechSynthesis.speak(utterance);listItem.classList.add('voice-speeching');utterance.onend=()=>{listItem.scrollIntoView({behavior:'smooth',block:'center'});listItem.classList.remove('voice-speeching');currentIndex++;saveReadingHistory(page.data.url,currentIndex,listItems.length,page.data.episodeCount);readNextItem()}}else{const selectItems=document.getElementById('ons-select1').querySelectorAll('ons-select-option');if(selectItems[selectItems.length-1]!==document.getElementById('ons-select1').querySelector('ons-select-option.selected')){document.getElementById('next-page-button').click()}else{document.getElementById('pause-button').style.display='none';document.getElementById('play-button').style.display='flex';window.reading=!1;if(window.screenWakeLock){window.screenWakeLock.release().then(()=>{window.screenWakeLock=null})}
saveReadingHistory(page.data.url,currentIndex+1,listItems.length,page.data.episodeCount)}}}
...
const data=await fetchData(ncode,'',i);aheadPages[url]=aheadPages[url]||{};aheadPages[url][i]=LZString.compress(JSON.stringify(data));localStorage.setItem('aheadPages',JSON.stringify(aheadPages));await new Promise(resolve=>setTimeout(resolve,10000))}}
