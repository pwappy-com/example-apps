...
await fetchDataAndDisplayByPage(ncode,status,currentPage);if(currentRow>0&&currentRow<totalRowCount){const detailList=document.getElementById('detail-list');const listItems=detailList.querySelectorAll('.list-item_p');listItems[currentRow-1].classList.add('voice-speeching');listItems[currentRow-1].scrollIntoView({behavior:'smooth',block:'center'})}}finally{setLoadingIndicator(!1);pageButtonControl(1,currentPage)}}
function なろう詳細共通(){}
...
function 読み上げ開始(event){const detailList=document.getElementById('detail-list');const listItems=detailList.querySelectorAll('.list-item_p');const page=detailList.closest('ons-page');let currentIndex=0;const voiceSpeechingItems=detailList.querySelectorAll('.voice-speeching');if(voiceSpeechingItems.length>0){currentIndex=Array.from(listItems).indexOf(voiceSpeechingItems[0])}
...
const icon=narouItem.querySelector('.narou-item-icon');if(readingStatus==='Reading'){icon.setAttribute('icon','fa-book-open');icon.style.color="#eb9d9d"}else if(readingStatus==='Done'){icon.setAttribute('icon','fa-check');icon.style.color="green"}})}
