function 設定値(){}
const fetchBase='https://script.google.com/macros/s/AKfycbxATXJgq0nqOizuxDoSiSHWsZlap8tOghw9eOAIzGynh0RDpTeELn7GiF9EW9rvVqAA9A/exec'
...
async function なろう詳細ページのShow(event){setLoadingIndicator(!0);try{const ncode=event.target.data.ncode;const title=event.target.data.title;const status=event.target.data.status;const episodeCount=event.target.data.episodeCount;setupPageInfo(title,status,episodeCount);await fetchDataAndDisplayByPage(ncode,status,1)}finally{setLoadingIndicator(!1);pageButtonControl(1,episodeCount)}}
function なろう詳細共通(){}
function setupPageInfo(title,status,episodeCount){const onsListHeader2=document.getElementById('ons-list-header2');onsListHeader2.textContent=title;const onsSelect1=document.getElementById('ons-select1');if(status==='短編'){onsSelect1.style.display='none'}else{const select1=onsSelect1.querySelector('.select-input');select1.innerHTML='';for(let i=1;i<=episodeCount;i++){const option=document.createElement('option');option.value=i;option.text=`ページ - ${i}`;select1.appendChild(option)}}}
...
page.addEventListener('show',作品一覧表示);page.addEventListener('destroy',function(event){page.removeEventListener('show',作品一覧表示)},{'once':!0});let onssegment1_element=document.querySelector('#ons-segment1');if(onssegment1_element){onssegment1_element.addEventListener('postchange',作品一覧表示);page.addEventListener('destroy',function(event){onssegment1_element.removeEventListener('postchange',作品一覧表示)},{'once':!0})}}})